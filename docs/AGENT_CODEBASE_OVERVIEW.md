...
cloud binding metadata while a game is local-only. BKE-5C is complete. BKE-5D is approved in
`PLAN_BKE_5D_RELEASE_AND_EXIT.md` as BKE-5D1 centralized default-off policy/hardening followed by
BKE-5D2 production activation and owner signoff. The initial owner-only rollout may validate after
deployment; the complete BKE-4/BKE-5 matrix remains required before access broadens. Youth
Equal-Play uses eight distinct periods but half-level foul and timeout windows, proving those
concepts must be separate. BKE-5B and BKE-5C remain internal until BKE-5D2. Clock/lineup
