...
Soccer Restarts R1-R2 widen the existing version-1 `soccer.team_event` reader to
...
state; Summary comparison exposes non-zero Throw-in and Goal-kick totals. R3
owns all writers, so no live UI can emit the new kinds yet.
