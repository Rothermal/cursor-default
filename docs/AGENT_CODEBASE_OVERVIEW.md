...
and coarse formation audit metadata; deploy it before using current team saves
because every current team save writes v2. Team Manage now has Rules/Formation
...
Soccer S23A extends only the team-settings foundation. Current logical team
...
matching SQL validation and only the coarse `lineup_defaults` audit key. Apply
068 before the S23A client saves team settings. S23B owns the Team Manage editor
and S23C owns one-time Player Setup consumption; no current live match behavior
uses the stored defaults yet. See `docs/REGRESSION_SOC_S23A_LINEUP_SETTINGS.md`.
