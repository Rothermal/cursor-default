...
detection, and mutation classification. `PlayerSetup` still rejects version-3 startup, so BKE-6B2
is next. See `REGRESSION_BKE_6B1_CLOCK_SETUP_CONTRACTS.md`.
BKE-6B2 through BKE-6E remain; rules versions 1-2, setup version 1, and existing games remain
...
| [`PLAN_BKE_6B_PRODUCTION_SETUP_AND_CLOCK.md`](PLAN_BKE_6B_PRODUCTION_SETUP_AND_CLOCK.md) | Detailed BKE-6B1 through BKE-6B4 plan for production rules/setup controls, opening lineup, canonical live-clock capture, expiration/recovery, and parking guards; anchored cloud and live lineup/equal-play controls remain later phases |
| [`REGRESSION_BKE_6B1_CLOCK_SETUP_CONTRACTS.md`](REGRESSION_BKE_6B1_CLOCK_SETUP_CONTRACTS.md) | BKE-6B1 evidence for device clock preferences, version-3 settings review/save warnings, strict setup-draft v2, shared workflow policies, and unchanged startup gate |
