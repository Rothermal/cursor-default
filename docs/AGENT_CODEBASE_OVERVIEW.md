...
Soccer S19A-S19B add the team-formation foundation and Team Manage editor
without changing match authority.
...
read-only access, and explicit stale-player cleanup on Formation-tab Save. S19C
owns one-time Player Setup prefill; see
`docs/REGRESSION_SOC_S19A_FORMATION_FOUNDATION.md` and
`docs/REGRESSION_SOC_S19B_FORMATION_EDITOR.md`.
