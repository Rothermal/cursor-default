...
Soccer Restarts R1 widens the existing version-1 `soccer.team_event` reader to
...
recognize both new kinds with the generic restart glyph. R2 owns richer review
labels/totals and R3 owns all writers, so no live UI can emit the new kinds yet.
