...
`PLAN_BKE_5_SETTINGS_AND_EVENT_ROLLOUT.md`. Its four planned slices cover source-audited immutable
profiles and version-1 compatibility, generic-table Basketball settings through fixed CAS RPCs,
mutation-free Event/Legacy setup with durable local-only sync policy, and evidence-gated production
opt-in. Youth Equal-Play uses eight distinct periods but half-level foul and timeout windows, proving
those concepts must be separate. BKE-5A through BKE-5C remain internal; BKE-5D is blocked on the
...
| [`PLAN_SOC_FIELD_TEST_BACKLOG.md`](PLAN_SOC_FIELD_TEST_BACKLOG.md) | Post-SOC-6 soccer field-test backlog. Owner-confirmed through `S20` (sideline layout, default role, Timeline shot edit, opponent-actor lock, finalize mismatch, header, placement, corner UI cleanup, upside-down flip counts, team formation lineup, throw-ins not in product). Do not implement from the backlog; expand one confirmed item at a time |
