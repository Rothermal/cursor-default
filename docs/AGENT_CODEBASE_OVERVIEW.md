...
| [`PLAN_SOC_FIELD_TEST_BACKLOG.md`](PLAN_SOC_FIELD_TEST_BACKLOG.md) | Post-SOC-6 soccer field-test backlog. Implemented recovery/UX items and remaining owner-confirmed ideas through `S21`; `S17`/`S20` restart capture and `S19` team formation now have separate approved plans. Do not implement remaining items directly from the backlog; expand one confirmed item at a time |
