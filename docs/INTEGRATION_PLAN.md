...
The **ASCII diagram below is historical (pre–018)** and shows `players.team_id` and sport on `teams` — **that is not the current schema**. Use **`supabase/migrations/`** and the **README** migration list (001–**038**) as the source of truth.
...
| **Roster** | `players` (global; `created_by`; optional `is_team_placeholder`); **`team_players`** junction; RPC-only guardian claims/removal and identity-only updates (**038**) — **no** `players.team_id` |
...
Policies are defined **per migration** (not duplicated here). Migrations `035` and `036` are the current team-access authority: team-derived reads require accepted membership; owner/admin/scorer can track through `can_track_team_games`; viewer is read-only; recorder writes are bound to a non-final team game and valid player; and corrections remain owner/admin actions. See those migrations and `docs/ACCESS_MATRIX.md`.
