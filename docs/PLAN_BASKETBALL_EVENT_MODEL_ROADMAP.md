...
BKE-5C setup/authority (BKE-5C1 through BKE-5C3 complete; BKE-5C4 remains), and BKE-5D
...
| BKE-5 | Built-in/personal/team/match Basketball settings, versioned rule profiles, mutation-free authority setup, and event-model rollout. **BKE-5A, BKE-5B, and BKE-5C1 through BKE-5C3 are implemented through migration 062; BKE-5C4 and BKE-5D remain** | BKE-4E | Defaults resolve with source metadata, setup fixes a complete immutable rules snapshot, local-only intent is durable, and the user opt-in ships after accepted evidence |
