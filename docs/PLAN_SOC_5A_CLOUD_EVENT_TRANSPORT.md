...
- `game_event_stream_checkpoints`, containing recorder event count, max sequence, exact
  event-id/revision set, stream fingerprint, and confirmation time;
...
7. The checkpoint RPC verifies cloud count, maximum sequence, and every event id/revision before
   recording the stream fingerprint.
...
any healthy parked soccer game enters the same queue, binds by its existing local UUID, snapshots
...
- Soccer event games never enter aggregate stat/shot replacement.
- Personal soccer rows are excluded from the legacy latest-aggregate-game hydrator until SOC-5B
  adds event-aware cloud resume.
...
- Unit-test participant snapshot mapping, cloud-team source links, deterministic checkpoints,
  healthy existing-game adoption, and diagnostic rejection.
