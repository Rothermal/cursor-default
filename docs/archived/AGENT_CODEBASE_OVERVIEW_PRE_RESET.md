> Historical snapshot before the current-product documentation reset.
> Not current setup guidance or an active task queue. See the repository README
> and docs/README.md for current entry points. Claims and unresolved notes below
> are preserved for audit, not newly verified.
...
BKE-6C2 adds a projection-driven tracked/optional-opponent boundary coordinator, changed-candidate
reuse of the C1 sheet, stale-safe atomic substitution/override/confirmation, and Start-before-review
gating; see `REGRESSION_BKE_6C2_BOUNDARY_EQUAL_PLAY.md`. BKE-6C3 adds atomic lineup-plus-role
capture, preset/custom/none positions, independent captains, late-bench return, replacement guidance,
...
three release-hardening slices in `PLAN_BKE_6E_RELEASE_HARDENING.md`; BKE-6E1 and BKE-6E2 are
implemented and BKE-6E3 exact-candidate owner smoke is next. Rules
...
quality disclosure, and eligible player/five-person plus-minus. BKE-6D2 adds sum-then-truncate
canonical seconds, lineup-entry appearances, DNP, plus-minus coverage provenance, and partial-rank
suppression without changing clockless manual-minute history. BKE-6D3 enables anchored
creation/binding through the existing one-recorder transport after both fresh capabilities while
preserving running sync, strict adoption, recovery uploads, and quarantine. BKE-6D4 adds migration
064 trusted anchored blockers/finalization, mode-aware Correct records/Resume game reopen, versioned
history, and strict prior-recorder parked-binding handoff behind additive fixed wrappers; existing
clockless RPC shapes, canonical versions, Soccer, and old mode-less reopen behavior stay unchanged.
BKE-6D1 is implemented with isolated Summary participation, quality, plus-minus, lineup, and
display-only clock detail. BKE-6D2 extends the canonical catalog with `bk_dnp` and signed `bk_pm`,
derives exact seconds after summing match participation milliseconds, carries manual/interval basis
and included/total-game metric coverage, shows partial eligible values only in individual review,
and suppresses incomplete comparative rankings. Existing canonical page envelopes remain exact and
no migration was added. BKE-6D3 now enables exact anchored setup through the existing v4
one-recorder transport after fresh access and both capability checks, with checkpoint-first
...
contract is detailed in `PLAN_BKE_6E_RELEASE_HARDENING.md`. BKE-6E1 adds no runtime behavior or
migration: `releaseEntryGuards.test.ts` pins every implementation consumer of the centralized
availability/creation policies. BKE-6E2 adds exact deployed build identity, prompt-based PWA
updates guarded by running-clock reload preparation, compact safe-area/reduced-motion behavior,
critical modal focus handling, and repeatable browser/PWA/rollback procedures. The single
`REGRESSION_BKE_6E_RELEASE.md` record separates automation, pending BKE-6E3 deployed evidence, and
...
Soccer S19A-S19C add team-formation defaults without changing match authority.
`src/lib/soccer/formation.ts` owns nine immutable 11v11/9v9/7v7 templates,
strict version-1 formation parsing, clone-safe editor transitions, stale-player
cleanup, and fail-closed broad-role setup mapping. Personal Soccer settings
remain schema version 1; S19 introduced logical `{ rules, formation }` team
settings and team schema version 2. Migration 065 adds strict
template/slot/UUID validation and coarse formation audit metadata. Team Manage
initially added Rules/Formation
tabs, fixed pitch slots, a shared accessible slot list, active-roster assignment,
read-only access, and explicit stale-player cleanup on Formation-tab Save.
Player Setup independently resolves current/cached team settings and a coherent
active roster, then applies a matching formation once to editable participant
drafts. Saved participants or recorder edits always suppress later prefill;
mismatch, invalid, unavailable-player, and settings failures use visible
roster-default fallbacks. See
`docs/REGRESSION_SOC_S19A_FORMATION_FOUNDATION.md` and
`docs/REGRESSION_SOC_S19B_FORMATION_EDITOR.md` and
`docs/REGRESSION_SOC_S19C_SETUP_PREFILL.md`.

Soccer S23A-S23C extend team settings, Team Manage, and Player Setup. Current logical team
settings are `{ rules, formation, lineupDefaults }`: readers normalize team
schema versions 1 and 2 with an empty sparse starter list, while all new saves
write exact team schema version 3. `src/lib/soccer/lineupDefaults.ts` owns the
version-1 stable-player starter ids, deterministic normalization, immutable
status changes, and complete-membership cleanup helpers. Migration 068 adds
matching SQL validation and only the coarse `lineup_defaults` audit key. Team
Manage exposes a list-driven Rules/Formation/Lineup Defaults tab set, role-
ordered active players, non-blocking lineup warnings, scorer/viewer read-only
review, inactive-member retention, and selected-tab-only cleanup after both
roster datasets load. `teamLineupPrefill.ts` composes one formation-first
setup transform: an applicable formation owns the complete starting group;
otherwise active saved starter ids apply while roster roles remain unchanged.
Saved participants and recorder edits suppress delayed prefill. See
`docs/REGRESSION_SOC_S23A_LINEUP_SETTINGS.md` and
`docs/REGRESSION_SOC_S23B_LINEUP_EDITOR.md` and
`docs/REGRESSION_SOC_S23C_SETUP_PREFILL.md`.
...
S24B-S24D still own the atomic target-lineup event, two-column live manager,
Timeline correction, and release hardening. Halftime is only the exact midpoint
of an even regulation-segment list, and replay/correction must re-derive it. See
`docs/PLAN_SOC_S24_LIVE_LINEUP_MANAGER.md`; do not implement S24 directly from
the field-test backlog.

Soccer Restarts R1-R4 widen the existing version-1 `soccer.team_event` reader to
Corner, Offside, Throw-in, and Goal kick. Restart events may carry one optional
`taker`; Offside retains one optional `offside_player`; contextual projection
rejects tracked participant identity on opponent events. Side projections add
Throw-ins and Goal kicks, and `src/lib/soccer/field.ts` provides a canonical,
direction-aware suggestion that never forces the saved kind. Review markers
recognize both new kinds with the generic restart glyph. Shared Timeline/Field
presentation names the kind, awarded side, and actor or explicit not-recorded
state; Summary comparison exposes non-zero Throw-in and Goal-kick totals. The
Field's component-local one-shot Restart arm routes its next canonical pitch
tap into the shared four-kind confirmation sheet without changing persisted
capture mode. The same sheet supports optional-location historical Add/Edit.
Tracked restart takers are on-field participants and opponent takers are labels;
projection remains fail-closed. The owner functional restart regression is
complete. `REGRESSION_SOC_RESTARTS.md` preserves the initial shared cloud
conflict-loop failure, PR #373's remediation, the post-fix completion replay,
and optional cached-reader/local-only evidence.
...
Existing local and cloud Soccer routes do not depend on development mode. Migration 049 introduced
the authenticated read-only capability handshake for the complete Soccer cloud boundary; S24A
migration 069 advances that exact contract for setup v2. Team Info, team setup deep links, and Soccer cloud-source
...
| [`PLAN_APP_THEMING.md`](../PLAN_APP_THEMING.md) | Approved roadmap for a device-local Settings -> App Light/Dark preference, semantic theme tokens, pre-React bootstrap, phased route/sport conversion, and release-gated Dark exposure; THM-1 through THM-6 are not started |
...
| [`PLAN_SOC_6_SUMMARY_AND_RELEASE.md`](../PLAN_SOC_6_SUMMARY_AND_RELEASE.md) / [`PLAN_SOC_6A_SUMMARY_FOUNDATION.md`](../PLAN_SOC_6A_SUMMARY_FOUNDATION.md) / [`PLAN_SOC_6B_DETAILED_MATCH_REVIEW.md`](../PLAN_SOC_6B_DETAILED_MATCH_REVIEW.md) / [`PLAN_SOC_6C_CANONICAL_AGGREGATES.md`](../PLAN_SOC_6C_CANONICAL_AGGREGATES.md) / [`PLAN_SOC_6D_SOCCER_SETTINGS.md`](../PLAN_SOC_6D_SOCCER_SETTINGS.md) / [`PLAN_SOC_6E_RELEASE_HARDENING.md`](../PLAN_SOC_6E_RELEASE_HARDENING.md) / [`PLAN_SOC_FIELD_TEST_BACKLOG.md`](../PLAN_SOC_FIELD_TEST_BACKLOG.md) / [`PLAN_SOC_RESTARTS.md`](../PLAN_SOC_RESTARTS.md) / [`REGRESSION_SOC_RESTARTS.md`](../REGRESSION_SOC_RESTARTS.md) / [`PLAN_SOC_S19_TEAM_FORMATION.md`](../PLAN_SOC_S19_TEAM_FORMATION.md) / [`REGRESSION_SOC_S19A_FORMATION_FOUNDATION.md`](../REGRESSION_SOC_S19A_FORMATION_FOUNDATION.md) / [`REGRESSION_SOC_S19B_FORMATION_EDITOR.md`](../REGRESSION_SOC_S19B_FORMATION_EDITOR.md) / [`REGRESSION_SOC_S19C_SETUP_PREFILL.md`](../REGRESSION_SOC_S19C_SETUP_PREFILL.md) / [`PLAN_SOC_S23_LINEUP_STATUS_DEFAULTS.md`](../PLAN_SOC_S23_LINEUP_STATUS_DEFAULTS.md) / [`REGRESSION_SOC_S23A_LINEUP_SETTINGS.md`](../REGRESSION_SOC_S23A_LINEUP_SETTINGS.md) / [`REGRESSION_SOC_S23B_LINEUP_EDITOR.md`](../REGRESSION_SOC_S23B_LINEUP_EDITOR.md) / [`REGRESSION_SOC_S23C_SETUP_PREFILL.md`](../REGRESSION_SOC_S23C_SETUP_PREFILL.md) / [`PLAN_SOC_S24_LIVE_LINEUP_MANAGER.md`](../PLAN_SOC_S24_LIVE_LINEUP_MANAGER.md) / [`REGRESSION_SOC_S24A_FROZEN_PRESET.md`](../REGRESSION_SOC_S24A_FROZEN_PRESET.md) | SOC-6A through SOC-6E3 implemented; owner-only production opt-in approved with post-deployment validation pending. Restart capture (`S17`/`S20`) is complete. Team formation S19 and lineup defaults S23 are implementation-complete with migrations 065/068 applied; deployed verification remains. S24A is implemented with migration 069 pending deployment; S24B-S24D own the live target-lineup manager and grouped correction |
| [`PLAN_BASKETBALL_EVENT_MODEL_ROADMAP.md`](../PLAN_BASKETBALL_EVENT_MODEL_ROADMAP.md) / [`PLAN_BKE_2_COMPLETE_EVENT_CAPTURE.md`](../PLAN_BKE_2_COMPLETE_EVENT_CAPTURE.md) / [`PLAN_BKE_3_EVENT_TIMELINE_AND_F13.md`](../PLAN_BKE_3_EVENT_TIMELINE_AND_F13.md) / [`PLAN_BKE_4_EVENT_CLOUD_CUTOVER.md`](../PLAN_BKE_4_EVENT_CLOUD_CUTOVER.md) / [`PLAN_BKE_4A_NEUTRAL_RPC_EXTRACTION.md`](../PLAN_BKE_4A_NEUTRAL_RPC_EXTRACTION.md) / [`PLAN_BKE_4B_BASKETBALL_TRANSPORT.md`](../PLAN_BKE_4B_BASKETBALL_TRANSPORT.md) / [`PLAN_BKE_4C_RECORDERS_AND_FINALIZATION.md`](../PLAN_BKE_4C_RECORDERS_AND_FINALIZATION.md) / [`PLAN_BKE_4D_SUMMARY_AUTHORITY.md`](../PLAN_BKE_4D_SUMMARY_AUTHORITY.md) / [`PLAN_BKE_4E_AGGREGATES_AND_RELEASE_READINESS.md`](../PLAN_BKE_4E_AGGREGATES_AND_RELEASE_READINESS.md) / [`PLAN_BKE_5_SETTINGS_AND_EVENT_ROLLOUT.md`](../PLAN_BKE_5_SETTINGS_AND_EVENT_ROLLOUT.md) / [`PLAN_BKE_5C_SETUP_AUTHORITY_AND_BINDING.md`](../PLAN_BKE_5C_SETUP_AUTHORITY_AND_BINDING.md) / [`PLAN_BKE_5D_RELEASE_AND_EXIT.md`](../PLAN_BKE_5D_RELEASE_AND_EXIT.md) / [`PLAN_BKE_6_CLOCK_AND_LINEUPS.md`](../PLAN_BKE_6_CLOCK_AND_LINEUPS.md) / [`PLAN_BKE_6C_LIVE_LINEUPS_AND_CORRECTIONS.md`](../PLAN_BKE_6C_LIVE_LINEUPS_AND_CORRECTIONS.md) / [`PLAN_BKE_6D_CLOUD_SUMMARY_AND_AGGREGATES.md`](../PLAN_BKE_6D_CLOUD_SUMMARY_AND_AGGREGATES.md) / [`REGRESSION_BKE_4C_FINALIZATION.md`](../REGRESSION_BKE_4C_FINALIZATION.md) / [`REGRESSION_BKE_4D_SUMMARY.md`](../REGRESSION_BKE_4D_SUMMARY.md) / [`REGRESSION_BKE_4E_AGGREGATES.md`](../REGRESSION_BKE_4E_AGGREGATES.md) / [`REGRESSION_BKE_4E_TRANSPORT.md`](../REGRESSION_BKE_4E_TRANSPORT.md) / [`REGRESSION_BKE_4E_DESTINATIONS.md`](../REGRESSION_BKE_4E_DESTINATIONS.md) / [`REGRESSION_BKE_4E_PLAYER_CAREER.md`](../REGRESSION_BKE_4E_PLAYER_CAREER.md) / [`REGRESSION_BKE_4E_RELEASE_READINESS.md`](../REGRESSION_BKE_4E_RELEASE_READINESS.md) / [`REGRESSION_BKE_5A_PROFILES.md`](../REGRESSION_BKE_5A_PROFILES.md) / [`REGRESSION_BKE_5B1_SETTINGS_FOUNDATION.md`](../REGRESSION_BKE_5B1_SETTINGS_FOUNDATION.md) / [`REGRESSION_BKE_5B2_PERSONAL_SETTINGS.md`](../REGRESSION_BKE_5B2_PERSONAL_SETTINGS.md) / [`REGRESSION_BKE_5B3_TEAM_SETTINGS.md`](../REGRESSION_BKE_5B3_TEAM_SETTINGS.md) / [`REGRESSION_BKE_5B4_UPGRADES_IMPORT.md`](../REGRESSION_BKE_5B4_UPGRADES_IMPORT.md) / [`REGRESSION_BKE_5B_SETTINGS.md`](../REGRESSION_BKE_5B_SETTINGS.md) / [`REGRESSION_BKE_6A1_RULES_SETUP.md`](../REGRESSION_BKE_6A1_RULES_SETUP.md) / [`REGRESSION_BKE_6A2_CLOCK_PROJECTION.md`](../REGRESSION_BKE_6A2_CLOCK_PROJECTION.md) / [`REGRESSION_BKE_6A3_LINEUP_PROJECTION.md`](../REGRESSION_BKE_6A3_LINEUP_PROJECTION.md) / [`REGRESSION_BKE_6C1_LIVE_SUBSTITUTIONS.md`](../REGRESSION_BKE_6C1_LIVE_SUBSTITUTIONS.md) / [`REGRESSION_BKE_6C2_BOUNDARY_EQUAL_PLAY.md`](../REGRESSION_BKE_6C2_BOUNDARY_EQUAL_PLAY.md) / [`REGRESSION_BKE_6C3_ROLES_AND_RECOVERY.md`](../REGRESSION_BKE_6C3_ROLES_AND_RECOVERY.md) / [`REGRESSION_BKE_6C4_LINEUP_CORRECTIONS.md`](../REGRESSION_BKE_6C4_LINEUP_CORRECTIONS.md) / [`REGRESSION_BKE_6D1_SUMMARY_DETAIL.md`](../REGRESSION_BKE_6D1_SUMMARY_DETAIL.md) / [`REGRESSION_BKE_6D2_EXACT_AGGREGATES.md`](../REGRESSION_BKE_6D2_EXACT_AGGREGATES.md) / [`REGRESSION_BKE_6D3_ANCHORED_CLOUD_TRANSPORT.md`](../REGRESSION_BKE_6D3_ANCHORED_CLOUD_TRANSPORT.md) / [`REGRESSION_BKE_6D4_FINALIZATION_AND_REOPEN.md`](../REGRESSION_BKE_6D4_FINALIZATION_AND_REOPEN.md) | Basketball migration onto the shared event model. BKE-1 through BKE-3, BKE-4A through BKE-4E, BKE-5A through BKE-5D2, BKE-6A through BKE-6D are implemented through migration 064. BKE-6E1 audit and BKE-6E2 release-surface hardening are implemented; BKE-6E3 deployment smoke and the complete live matrix still gate broader access |
...
| [`PLAN_BKE_6E_RELEASE_HARDENING.md`](../PLAN_BKE_6E_RELEASE_HARDENING.md) / [`REGRESSION_BKE_6E_RELEASE.md`](../REGRESSION_BKE_6E_RELEASE.md) | BKE-6E1 audit and BKE-6E2 build/PWA/responsive/accessibility/rollback hardening are implemented through migration 064; BKE-6E3 exact-candidate owner smoke is next |
...
| [`PLAN_SOC_FIELD_TEST_BACKLOG.md`](../PLAN_SOC_FIELD_TEST_BACKLOG.md) | Post-SOC-6 soccer field-test backlog. Implemented recovery/UX items and remaining owner-confirmed ideas through `S26`; `S17`/`S20` restart capture and owner functional verification are complete, `S19` team formation plus migration 065 and `S23` lineup defaults plus migration 068 are implementation-complete pending deployed verification. Do not implement remaining items directly from the backlog; expand one confirmed item at a time |
