...
dedicated team roster/schedule routes, and season management. THM-3C1 covers the
Cloud Games list, THM-3C2 covers the Game Info host, and THM-3C3 covers its
imported review panels. THM-3D covers both sports' aggregate/player renderers,
shared legacy tables, and all five statistics route hosts. THM-3 implementation
is complete; documented deployed checks remain open and production Dark stays
gated until the THM-6 release audit.
...
also affect other sports; sport-specific Soccer setup remains THM-5 scope.
See [THM-4 setup verification](REGRESSION_THM_4_SETUP_SURFACES.md) for evidence
and untested release cases. Live tracking, court/corrections, and Summary are
