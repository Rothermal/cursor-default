...
| 11a6.3 | Record one-, two-, and three-attempt trips; try one-and-one outside its configured post-foul bonus window; then record and delete a valid one-and-one first miss | Award/rules and technical/foul context mismatches are rejected; attempt positions are stable; the historical first miss keeps the one-and-one closed after correction and no unearned second attempt is created |
...
| 11a6.5 | Decrement a foul linked to a trip plus official and automatic-threshold ejections | Preview reports personal/team/technical, bonus, disqualification, unlink, and automatic-removal effects; correction unlinks the official ejection and removes an invalidated automatic ejection atomically; Restore reverses the full batch |
