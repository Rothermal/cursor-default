...
| 11b.2 | Initialize a soccer event stream using kickoff with a resolved `sportGameState` setup | Opening lineup and period start append as one coherent batch; clock remains paused until explicit Start (S6) |
...
| 11c.8 | Confirm kickoff, wait, then press Start | Opening lineup and first period begin with clock paused at zero; waiting accrues no minutes; Start begins timing from the click (S6) |
...
| 11l.2 | Recorder B opens the same Cloud Game and confirms **Start your own independent recorder stream** | B receives the immutable setup/participants and two new kickoff events with clock paused, bound to the same game id; no A event is copied; B explicitly starts the clock (S6) |
...
| 11z.2a | Revisit Player Setup with an already-loaded roster while offline, then retry a fresh failed cloud-roster load | The loaded roster remains usable with safe role fallbacks; a required failed load shows Retry and succeeds after connectivity returns |
| 11z.2b | Save a matching team formation, start a fresh cloud-team match, and continue to Player Setup | Assigned active players prefill as starters with the slot's broad role; unassigned active players remain selected Bench with their roster role; tactical slot labels are not stored |
| 11z.2c | Edit a participant before a delayed team-settings retry succeeds, then trigger focus/online refreshes | The recorder's draft remains unchanged and the team formation is not applied later |
| 11z.2d | Use a mismatched formation count, an unavailable assigned player, then an invalid/unavailable settings response | Mismatch uses roster defaults with a warning; valid assignments apply without fabricating stale players; settings failure keeps manual setup usable and offers Retry |
