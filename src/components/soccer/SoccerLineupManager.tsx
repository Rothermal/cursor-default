...
export default function SoccerLineupManager({ state, recorderUserId, initialParticipantId, busy, onApply, onClose, eventId = null }: {
  ...
  if (!projection) return <div role="alert" className="fixed inset-0 z-50 bg-surface p-4">Earlier lineup history is unavailable.<button type="button" onClick={onClose} className="ml-3 underline">Close</button></div>
  ...
  const changeRole = (id: string, role: SoccerRole) => {
    editDraft({ type: 'role', participantId: id, role }); setError(null)
  }
  const apply = () => {
    ...
    if (soccerLineupSubmitStep(result.ok, target.length, projection.currentRules.maxOnFieldPlayers, confirmShort) === 'confirm') { setConfirmShort(true); return }
    if (onApply(result)) onClose()
  }
  const names = (ids: string[]) => ids.map(id => projection.participants[id]?.displayName ?? id).join(', ')
  return <div style={{ margin: 0 }} className="fixed inset-0 z-50 flex items-center justify-center bg-overlay/[0.5] p-0 sm:p-3">
    <div ref={dialogRef} role="dialog" aria-modal="true" aria-labelledby="lineup-manager-title" tabIndex={-1}
      className="flex h-[100dvh] w-full max-w-3xl flex-col bg-surface text-content sm:h-[94dvh] sm:rounded-lg">
      ...
      <div className="flex shrink-0 flex-wrap gap-2 border-b border-line p-3">
        <button type="button" disabled={blocked} onClick={() => preset('opening_lineup')} className="rounded-md border border-line-strong px-3 py-2 text-xs font-semibold disabled:bg-control-disabled disabled:text-content-disabled">Opening Lineup</button>
        {context && soccerLineupPreset(context, 'team_default') && <button type="button" disabled={blocked} onClick={() => preset('team_default')} className="rounded-md border border-line-strong px-3 py-2 text-xs font-semibold disabled:bg-control-disabled disabled:text-content-disabled">Team Default</button>}
        <button type="button" onClick={reset} disabled={busy} title={eventId ? 'Reset to recorded target' : 'Reset to current lineup'} aria-label={eventId ? 'Reset to recorded target' : 'Reset to current lineup'} className="grid h-9 w-9 place-items-center rounded-md border border-line-strong"><RotateCcw size={17} /></button>
      </div>
      <div className="min-h-0 flex-1 overflow-y-auto p-2 sm:p-3">
        {original && <p className="mb-2 text-xs text-content-muted">{soccerEventTimeLabel(original, soccerPeriodTimings(base))} · Revision {original.revision}</p>}
        <div className="grid grid-cols-2 gap-2 sm:gap-4">
          {[true, false].map(onField => <section key={String(onField)} className="min-w-0">
            <h3 className="sticky top-0 z-10 border-b border-line-strong bg-surface py-2 text-xs font-bold">{onField ? `ON FIELD ${target.length}/${projection.currentRules.maxOnFieldPlayers}` : `BENCH ${participants.length - target.length}`}</h3>
            {participants.filter(p => targetIds.has(p.participantId) === onField).map(p => {
              ...
              return <div key={p.participantId} className="min-w-0 border-b border-line py-2">
                <button type="button" disabled={blocked || (!onField && !!reason)}
                  ...
                  onClick={() => { editDraft({ type: 'move', participantId: p.participantId, role }); setError(null) }}
                  className="flex h-10 w-full min-w-0 items-center gap-1 text-left text-xs disabled:bg-control-disabled disabled:text-content-disabled">
                ...
                <>
                  <select ref={p.participantId === initialParticipantId ? focusRef : undefined} aria-label={`${onField ? 'Role' : 'Entry role'} for ${p.displayName}`} value={role.group} disabled={blocked || (!onField && !!reason)}
                    ...
                    className="h-9 w-full min-w-0 rounded border border-line-strong bg-surface px-1 text-xs">
                    {roles.map(([value, label]) => <option key={value} value={value}>{label}</option>)}
                  </select>
                  {role.group === 'custom' && <input aria-label={`Custom role for ${p.displayName}`} value={role.label ?? ''} disabled={blocked} onChange={event => changeRole(p.participantId, { ...role, label: event.target.value })} className="bg-surface mt-1 h-9 w-full min-w-0 rounded border border-line-strong px-1 text-xs" />}
                </>
                {!onField && <p className="truncate text-[11px] text-content-muted" title={reason ?? 'Role on entry'}>{reason ?? 'Role on entry'}</p>}
              </div>
            })}
          </section>)}
        </div>
        {unavailable.length > 0 && <div className="mt-3 border-l-4 border-warning-line bg-warning p-2 text-xs text-warning-content">{unavailable.map(item => <p key={item.participantId}>Unavailable in preset: {item.name} ({item.reason})</p>)}</div>}
        <div className="mt-3 space-y-1 border-t border-line pt-3 text-xs" aria-live="polite">
          <p className="font-semibold">{sourceNames[source]}</p>
          <p>On field: {target.length}/{projection.currentRules.maxOnFieldPlayers} · Goalkeepers: {target.filter(entry => entry.role.group === 'goalkeeper').length}/1</p>
          {preview.ok ? <>
          ...
          </> : <p className="text-warning-content">{preview.message}</p>}
        </div>
      </div>
      <footer className="shrink-0 space-y-2 border-t border-line bg-surface p-3">
        {latestProjection?.clock.running && <p role="status" className="text-sm text-warning-content">Pause clock to manage lineup</p>}
        {error && <p role="alert" className="text-sm text-danger-content">{error}</p>}
        {confirmShort && <p role="alert" className="text-sm font-semibold text-warning-content">Play short-handed with {target.length} of {projection.currentRules.maxOnFieldPlayers} players?</p>}
        <div className="flex gap-2"><button type="button" onClick={confirmShort ? () => setConfirmShort(false) : onClose} className="min-h-11 flex-1 rounded-md border border-line-strong px-2 text-sm font-semibold">{confirmShort ? 'Keep Editing' : 'Cancel'}</button>
          <button type="button" disabled={blocked || !preview.ok} onClick={apply} className="min-h-11 flex-1 rounded-md bg-success px-2 text-sm font-bold text-content disabled:bg-control-disabled disabled:text-content-disabled">{confirmShort ? 'Confirm Short-handed' : 'Apply Lineup'}</button></div>
      </footer>
    </div>
  </div>
}
