...
export default function SoccerShootoutManagementDialog(props: Props) {
  ...
  return <div className="fixed inset-0 z-50 flex items-end justify-center bg-overlay/[0.45] sm:items-center" onClick={props.onClose}><div role="dialog" aria-modal="true" aria-label={title} className="max-h-[92vh] w-full overflow-y-auto rounded-t-lg bg-surface sm:max-w-lg sm:rounded-lg" onClick={event => event.stopPropagation()}><header className="sticky top-0 z-10 flex min-h-14 items-center border-b border-line bg-surface px-4"><h2 className="flex-1 font-bold text-content">{title}</h2><button type="button" onClick={props.onClose} className="grid h-9 w-9 place-items-center text-content-muted" aria-label="Close" title="Close"><X size={20} /></button></header><div className="p-4">{props.kind === 'card' ? <CardForm {...props} /> : props.kind === 'goalkeeper' ? <GoalkeeperForm {...props} /> : <EligibilityForm {...props} />}</div></div></div>
}
...
function SideChoice({ side, onSide }: { side: 'tracked' | 'opponent'; onSide: (side: 'tracked' | 'opponent') => void }) { const { state } = useGame(); return <Field label="Side"><div className="grid grid-cols-2 rounded-md bg-control p-1"><Choice active={side === 'tracked'} label={gameSideDisplayName(state.gameInfo, 'tracked')} onClick={() => onSide('tracked')} /><Choice active={side === 'opponent'} label={gameSideDisplayName(state.gameInfo, 'opponent')} onClick={() => onSide('opponent')} /></div></Field> }
function Field({ label, children }: { label: string; children: ReactNode }) { return <section><h3 className="mb-2 text-xs font-bold uppercase text-content-muted">{label}</h3>{children}</section> }
...
function Submit({ busy, label, onClick }: { busy: boolean; label: string; onClick: () => void }) { return <button type="button" disabled={busy} onClick={onClick} className="min-h-12 w-full rounded-md bg-success text-sm font-bold text-content disabled:bg-control-disabled disabled:text-content-disabled">{label}</button> }
function ErrorText({ children }: { children: ReactNode }) { return <p className="rounded-md border border-danger-line bg-danger px-3 py-2 text-sm text-danger-content">{children}</p> }
