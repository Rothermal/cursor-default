  ...
  return <details className="border-y border-line py-3">
    ...
    <fieldset disabled={disabled} className="mt-3 min-w-0 space-y-3">
      ...
      {goal && <>
        ...
        {!shootout && <Approach location={location} />}
      </>}
      {children}
    </fieldset>
  </details>
...
export function SoccerShotDetailsReview({ event, collapsed = false }: { event: GameEvent; collapsed?: boolean }) {
  ...
  const content = <div className="space-y-3 py-3 text-sm text-content">
    ...
    {soccerPlacementAllowed(event.eventType, event.payload) && <>
      ...
      {event.eventType !== 'soccer.shootout_kick' && <Approach location={event.location} />}
    </>}
  </div>
...
function GoalMouth({ placement, onChange }: { placement: SoccerGoalPlacement | null; onChange?: (value: SoccerGoalPlacement) => void }) {
  return <figure className="m-0 min-w-0">
    <svg viewBox="0 0 100 34" role="img" aria-label="Goal mouth, facing the goal" className={`block aspect-[100/34] w-full rounded-sm border-2 border-pitch-line bg-pitch-surface ${onChange ? 'cursor-crosshair' : ''}`}
      onClick={onChange ? event => { const b = event.currentTarget.getBoundingClientRect(); if (!b.width || !b.height) return; onChange({ x: Math.max(0, Math.min(1, (event.clientX - b.left) / b.width)), y: Math.max(0, Math.min(1, (event.clientY - b.top) / b.height)) }) } : undefined}>
      ...
      {placement && <circle cx={placement.x * 100} cy={placement.y * 34} r="2" fill="rgb(var(--pitch-line))" stroke="rgb(var(--pitch-surface))" strokeWidth="0.8" />}
    </svg>
...
function Approach({ location }: { location: GameEventLocation | null }) {
  const approach = soccerShotApproach(location)
  if (!approach) return <p className="text-xs text-content-muted">Approach angle unavailable</p>
  return <figure className="m-0">
    <svg viewBox="0 0 100 64" role="img" aria-label={`Shot approach ${approach.degrees} degrees`} className="block aspect-[100/64] w-full rounded-sm border border-pitch-line bg-pitch-surface">
      ...
      <line x1={approach.origin.x * 100} y1={approach.origin.y * 64} x2={approach.goal.x * 100} y2="32" stroke="rgb(var(--pitch-line))" strokeWidth="1" strokeDasharray="2 1" />
      <circle cx={approach.origin.x * 100} cy={approach.origin.y * 64} r="1.5" fill="rgb(var(--pitch-line))" />
    </svg>
    <figcaption className="mt-1 text-xs text-content-muted">Approach: {approach.degrees} degrees (diagram estimate)</figcaption>
  </figure>
}
