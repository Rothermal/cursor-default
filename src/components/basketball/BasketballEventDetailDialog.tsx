...
export default function BasketballEventDetailDialog({
  ...
  return (
    <div className="fixed inset-0 z-[60] flex justify-center bg-overlay/[0.45] sm:items-center sm:p-4" onClick={onClose}>
      <section
        role="dialog"
        aria-modal="true"
      ...
      >
        <header className="flex shrink-0 items-start justify-between gap-3 border-b border-line px-4 py-3">
          <div className="min-w-0">
            <p className="break-words text-xs font-semibold uppercase text-content-muted">{review.periodLabel}</p>
            <h2 id="basketball-event-detail-title" className="break-words text-lg font-bold text-content">{review.title}</h2>
            <p className="mt-0.5 break-words text-sm text-content-muted">
              {captureLabel ?? formatRecordedAt(review.event.occurredAt)}
            </p>
          </div>
          <button ref={closeRef} type="button" onClick={onClose} className="flex h-10 w-10 shrink-0 items-center justify-center rounded-md border border-line text-content-muted" aria-label="Close event detail">
        ...
        <div className="min-h-0 flex-1 overflow-y-auto">
          ...
          <section className="border-b border-line px-4 py-4">
            <h3 className="text-xs font-semibold uppercase text-content-muted">{detailHeading(review)}</h3>
          ...
          <dl className="divide-y divide-line px-4 py-3 text-xs">
        ...
        <footer className="flex shrink-0 gap-2 border-t border-line px-4 py-3">
          ...
          {onRemove && <button type="button" onClick={onRemove} className="flex min-h-11 flex-1 items-center justify-center gap-2 rounded-md border border-danger-line bg-danger text-sm font-bold text-danger-content"><Trash2 size={17} aria-hidden />Remove</button>}
...
function DetailCell({ label, value }: { label: string; value: string }) {
  return <div className="min-w-0 border-r border-line px-4 py-3 last:border-r-0"><p className="text-xs font-semibold uppercase text-content-muted">{label}</p><p className="mt-1 break-words text-sm font-semibold text-content">{value}</p></div>
}

function DetailRow({ label, value }: { label: string; value: string }) {
  return <div className="grid grid-cols-[6rem_minmax(0,1fr)] gap-3 py-2"><dt className="font-semibold text-content-muted">{label}</dt><dd className="break-all text-content">{value}</dd></div>
}
