      ...
      >
        <header className="flex items-center justify-between gap-3 border-b border-line px-4 py-3">
          <div className="min-w-0">
            <h2 id="basketball-steal-turnover-title" className="text-base font-bold text-content">Steal + turnover</h2>
            <p className="truncate text-xs text-content-muted">Steal: {stealerLabel}</p>
          </div>
          <button type="button" onClick={onClose} className="flex h-9 w-9 shrink-0 items-center justify-center rounded-lg border border-line text-content-muted" aria-label="Close steal and turnover">
        ...
        >
          <fieldset>
            <legend className="mb-2 text-sm font-semibold text-content">Turnover by {turnoverTeamName}</legend>
            <div className="max-h-56 space-y-1 overflow-y-auto rounded-lg border border-line p-1">
              {[{ id: 'team', label: `${turnoverTeamName} team` }, ...candidates.map(candidate => ({ id: candidate.playerId, label: candidate.label })), { id: 'unknown', label: 'Unknown player' }].map(option => (
                <label key={option.id} className={`flex min-h-10 cursor-pointer items-center gap-3 rounded-md px-3 py-2 text-sm ${selection === option.id ? 'bg-surface-muted font-semibold text-content' : 'text-content'}`}>
          ...
          <div className="flex justify-end gap-2 border-t border-line pt-3">
            ...
            <button type="submit" disabled={target() === null} className="btn-primary inline-flex items-center gap-2 px-4 py-2 text-sm disabled:bg-control-disabled disabled:text-content-disabled">
