      ...
      >
        <header className="flex items-center justify-between gap-3 border-b border-line px-4 py-3">
          <div className="min-w-0">
            <h2 id="basketball-free-throw-trip-title" className="text-base font-bold text-content">Awarded free throws</h2>
            <p className="truncate text-xs text-content-muted">
              {teamName} · {trip.oneAndOne ? 'One-and-one' : `${trip.maximumAttempts} shot${trip.maximumAttempts === 1 ? '' : 's'}`}
            </p>
          </div>
          <button type="button" onClick={onClose} className="flex h-9 w-9 shrink-0 items-center justify-center rounded-lg border border-line text-content-muted" aria-label="Close free-throw trip">
        ...
        <div className="space-y-4 px-4 py-4">
                ...
                return (
                  <span
                    ...
                    className={`flex h-7 min-w-7 items-center justify-center rounded-md border px-1 text-xs font-bold ${
                      attempt?.deleted
                        ? 'border-line bg-surface-muted text-content-subtle line-through'
                        : attempt?.made
          ...
          {!trip.open && (
            <p className="rounded-lg border border-warning-line bg-warning px-3 py-2 text-sm text-warning-content">
          ...
          <div className="grid grid-cols-2 gap-2">
            <button type="button" onClick={() => onRecord(playerId, false)} disabled={!trip.open || !playerId} className="inline-flex min-h-12 items-center justify-center gap-2 rounded-lg bg-danger px-4 py-2 text-sm font-bold text-danger-content active:scale-95 disabled:bg-control-disabled disabled:text-content-disabled">
              <X size={17} aria-hidden />
              Miss
            </button>
            <button type="button" onClick={() => onRecord(playerId, true)} disabled={!trip.open || !playerId} className="inline-flex min-h-12 items-center justify-center gap-2 rounded-lg bg-success px-4 py-2 text-sm font-bold text-success-content active:scale-95 disabled:bg-control-disabled disabled:text-content-disabled">
          ...
          <div className="flex items-center justify-between gap-2 border-t border-line pt-3">
            <button type="button" onClick={onRemove} className="inline-flex min-h-10 items-center gap-2 rounded-lg px-2 text-sm font-semibold text-danger-content">
