    ...
    <div className="space-y-8 py-5">
      <section aria-labelledby="basketball-result-heading">
        ...
        <div className="mt-3 flex flex-wrap items-center gap-3">
          <span className="rounded-md bg-control px-3 py-2 text-sm font-bold text-content">
            {result.resultLabel}
          </span>
          <span className="text-sm text-content-muted">{result.gameStateLabel}</span>
        </div>
        <dl className="mt-4 grid gap-3 border-y border-line py-4 sm:grid-cols-2 lg:grid-cols-3">
      ...
      <section aria-labelledby="basketball-periods-heading">
        ...
        <div className="mt-3 overflow-x-auto border-y border-line">
          <table className="w-full min-w-[420px] text-sm">
            <thead className="bg-control text-xs uppercase text-content-subtle">
              <tr>
        ...
        <div className="mt-3 divide-y divide-line border-y border-line">
          {comparisons.map(section => (
            <div key={section.id} className="py-4">
              <h3 className="text-sm font-bold text-content">{section.label}</h3>
              <div className="mt-2 space-y-2">
                {section.rows.map(row => (
                  <div key={row.id} className="grid grid-cols-[1fr_minmax(110px,2fr)_1fr] items-center gap-3 text-sm">
                    <strong className="text-left tabular-nums text-content">
                      {formatComparison(row.tracked, row.trackedAttempted)}
                    </strong>
                    <span className="text-center text-content-subtle">{row.label}</span>
                    <strong className="text-right tabular-nums text-content">
      ...
      <section aria-labelledby="basketball-leaders-heading">
        ...
        {leaders.length > 0 ? (
          <div className="mt-3 grid gap-px overflow-hidden rounded-md border border-line bg-control sm:grid-cols-2 lg:grid-cols-3">
            {leaders.map(category => (
              <div key={category.id} className="bg-surface p-4">
                <p className="text-xs font-semibold uppercase text-content-subtle">{category.label}</p>
          ...
          <p className="mt-3 text-sm text-content-subtle">No positive player leaders recorded.</p>
        )}
      </section>
    </div>
  )
...
function SectionHeading({ id, title }: { id: string; title: string }) {
  return <h2 id={id} className="text-lg font-bold text-content">{title}</h2>
}
...
function PeriodRow({ name, values, total }: { name: string; values: number[]; total: number }) {
  return (
    <tr className="bg-surface">
      <th className="px-3 py-3 text-left font-semibold text-content">{name}</th>
      {values.map((value, index) => (
        <td key={index} className="px-3 py-3 text-center tabular-nums text-content">{value}</td>
      ))}
      <td className="px-3 py-3 text-center font-bold tabular-nums text-content">{total}</td>
    </tr>
  )
}
