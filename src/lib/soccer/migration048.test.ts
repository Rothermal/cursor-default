...
describe('migration 048 sport settings contracts', () => {
  ...
  it('enforces active manager team writes and emits the existing audit event', () => {
    ...
    const updateIndex = sql.indexOf('update public.team_sport_settings')
    const insertIndex = sql.indexOf('insert into public.team_sport_settings')
    const auditIndex = sql.indexOf('perform public.record_access_audit_event')
    const appliedIndex = sql.indexOf("'status', 'applied'", auditIndex)
    expect(updateIndex).toBeGreaterThan(-1)
    expect(insertIndex).toBeGreaterThan(-1)
    expect(auditIndex).toBeGreaterThan(updateIndex)
    expect(auditIndex).toBeGreaterThan(insertIndex)
    expect(appliedIndex).toBeGreaterThan(auditIndex)
  })
})
