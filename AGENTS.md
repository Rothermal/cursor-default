...
- **Settings routing (NAV-2):** `/settings` and `/settings/account` show account status, `/settings/app` owns enabled sport toggles, `/settings/sports` is the sport settings index, `/settings/sports/basketball` owns basketball-only preferences, `/settings/data` owns local parked-game import/export, cloud shortcuts, and Seasons, and `/settings/advanced` owns player merge plus destructive data-management tools. Legacy `#/admin` redirects to `#/settings`.
