...
- Game state uses local multi-game parking: `statkeeper_games_manifest` tracks `activeLocalGameId` plus parked summaries, each game is stored at `statkeeper_game:{localGameId}`, and `statkeeper_game` remains a legacy/active mirror. P0 parks/resumes multiple local games across sports; the full offline sync queue is still planned — see `docs/PLAN_MULTI_GAME_PARKING.md`. To reset state, use the "New Game" flow or clear localStorage.
