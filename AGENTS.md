...
- Game state uses local multi-game parking: `statkeeper_games_manifest` tracks `activeLocalGameId` plus parked summaries, each game is stored at `statkeeper_game:{localGameId}` with per-record dirty/revision sync metadata, and `statkeeper_game` remains a legacy/active mirror. P0 parks/resumes multiple local games across sports; P1 drains dirty parked games through the local sync queue. P2 cloud hardening is still planned — see `docs/PLAN_MULTI_GAME_PARKING.md`. To reset state, use the "New Game" flow or clear localStorage.
