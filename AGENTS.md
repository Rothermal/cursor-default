...
- **Auth (AUTH-1):** Supabase auth supports email/password plus Google OAuth. `AuthContext.signInWithGoogle()` uses `signInWithOAuth({ provider: 'google', redirectTo })`; redirect URL is `window.location.origin + window.location.pathname` so local dev returns to `http://localhost:5173/` and GitHub Pages returns to `/cursor-default/`. Google/Supabase provider setup is external; migration `034_google_auth_profile_defaults.sql` updates new-user profile defaults for Google metadata. Manual Google linking is deferred to AUTH-2.
