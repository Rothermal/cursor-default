...
- [ ] **Soccer live lineup manager (S24)** - S24A implements setup v2 and frozen Team Default with migration 069. S24B adds atomic target transitions, exact halftime/count/interval validation, stale-preview rejection, and historical correction primitives. S24C adds the live two-column manager, presets, and short-handed confirmation; S24D remains for grouped Timeline correction ([plan](docs/PLAN_SOC_S24_LIVE_LINEUP_MANAGER.md), [S24A regression](docs/REGRESSION_SOC_S24A_FROZEN_PRESET.md), [S24B regression](docs/REGRESSION_SOC_S24B_TARGET_TRANSITION.md))
