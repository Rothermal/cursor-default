...
- [ ] **Basketball event-model migration (BKE-1C through BKE-6)** — BKE-1B provides the complete internally registered event catalog and fail-closed authority boundary; BKE-1C is approved as three slices, starting with the checked command/setup foundation in BKE-1C1 ([roadmap](docs/PLAN_BASKETBALL_EVENT_MODEL_ROADMAP.md), [BKE-1C plan](docs/PLAN_BKE_1C_COURT_EVENTS.md))
