...
- [ ] **Soccer production enablement sign-off (SOC-6E3)** - the centralized production policy now offers Soccer as a device-local opt-in while preserving existing records when disabled; deployed GitHub Pages, PWA, role, cloud, multi-sport, and Basketball evidence remains to be recorded ([plan](docs/PLAN_SOC_6E_RELEASE_HARDENING.md), [release matrix](docs/REGRESSION_SOC_6E_RELEASE.md))

### What's Next

- [ ] **Soccer SOC-6E release sign-off** - record the preliminary and final deployed operator passes against migration 049; the production build is opt-in and the device default remains off ([release matrix](docs/REGRESSION_SOC_6E_RELEASE.md), [SOC-6 plan](docs/PLAN_SOC_6_SUMMARY_AND_RELEASE.md))
