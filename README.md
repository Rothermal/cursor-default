...
- [ ] **Basketball event-model migration (BKE-1C through BKE-6)** — BKE-1B now provides the complete internally registered event catalog and fail-closed authority boundary; next migrate court capture commands onto that foundation in BKE-1C ([roadmap](docs/PLAN_BASKETBALL_EVENT_MODEL_ROADMAP.md), [BKE-1B plan](docs/PLAN_BKE_1B_BASKETBALL_EVENT_FOUNDATION.md))
