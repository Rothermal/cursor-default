...
- [ ] **Basketball event-model migration (BKE-1C through BKE-6)** — BKE-1B provides the complete internally registered event catalog and fail-closed authority boundary; BKE-1C1 adds development-only local creation intent, checked commands, immutable participants, and atomic Period 1 start; BKE-1C2 routes the existing court popup through atomic projected events while preserving legacy Basketball; BKE-1C3 grouped corrections and parity proof are next ([roadmap](docs/PLAN_BASKETBALL_EVENT_MODEL_ROADMAP.md), [BKE-1C plan](docs/PLAN_BKE_1C_COURT_EVENTS.md))
