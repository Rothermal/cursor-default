...
- [x] **Basketball release hardening (BKE-5D1)** - centralized internal/opt-in creation policy, strict default-off device preference, separate Tracker settings UI, setup plus atomic commit guards, and existing-game continuation invariants; production remains internal ([plan](docs/PLAN_BKE_5D_RELEASE_AND_EXIT.md), [regression](docs/REGRESSION_BKE_5_SETTINGS_AND_ROLLOUT.md))
- [ ] **Basketball settings and event rollout (BKE-5A-5D)** - BKE-5A through BKE-5D1 are implemented; BKE-5D2 production activation and owner signoff remain, with a broader-release evidence gate ([plan](docs/PLAN_BKE_5_SETTINGS_AND_EVENT_ROLLOUT.md), [BKE-5D plan](docs/PLAN_BKE_5D_RELEASE_AND_EXIT.md))
