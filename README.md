...
- [ ] **Basketball setup authority and binding policy (BKE-5C)** - C1 implements account-scoped reload-safe drafts, mutation-free Basketball entry, deferred setup values, and rollback-safe one-step local commit; C2-C4 retain exact rule freeze, durable local-only policy, and guarded later cloud binding ([plan](docs/PLAN_BKE_5C_SETUP_AUTHORITY_AND_BINDING.md), [C1 regression](docs/REGRESSION_BKE_5C1_SETUP_DRAFT.md))
