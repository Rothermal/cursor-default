...
- [ ] **Basketball event-model migration (BKE-2 through BKE-6)** — BKE-1 is complete: the internally registered catalog, fail-closed authority boundary, local creation/setup, atomic court popup capture, grouped Recent Events, newest-first undo/restore, and dependency-aware Clear Chart are implemented while legacy Basketball remains unchanged. BKE-2 direct live actions are next ([roadmap](docs/PLAN_BASKETBALL_EVENT_MODEL_ROADMAP.md), [completed BKE-1C plan](docs/PLAN_BKE_1C_COURT_EVENTS.md))
