...
- [ ] **Basketball settings and event rollout (BKE-5A-5D)** - BKE-5A through BKE-5C are implemented; BKE-5D is approved as separate default-off policy-hardening and production-activation slices with owner post-deployment validation and a broader-release evidence gate ([plan](docs/PLAN_BKE_5_SETTINGS_AND_EVENT_ROLLOUT.md), [BKE-5D plan](docs/PLAN_BKE_5D_RELEASE_AND_EXIT.md))
