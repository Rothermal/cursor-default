...
- [ ] **Basketball event-model migration (BKE-0 through BKE-6)** — required redesign that unifies counters, action log, shot records, linked assists/rebounds, editing, cloud authority, canonical aggregates, layered settings, F13, and optional clock/lineup tracking on the shared event platform while preserving historical games; BKE-0 product-model Q&A is pending ([roadmap](docs/PLAN_BASKETBALL_EVENT_MODEL_ROADMAP.md))
