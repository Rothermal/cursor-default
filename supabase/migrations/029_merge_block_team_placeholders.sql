-- Re-apply merge RPCs with is_team_placeholder guard (requires migration 028).
-- Team stat placeholder rows must never be merged.
...
BEGIN
  ...
  IF EXISTS (
    ...
    RAISE EXCEPTION 'merge_players_preview: team stat placeholders cannot be merged';
  END IF;
