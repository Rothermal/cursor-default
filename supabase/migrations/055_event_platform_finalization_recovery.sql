-- BKE-4A4: sport-neutral finalization, publication recovery, and permanent Soccer wrappers.
...
create or replace function public.can_manage_event_game(
  p_sport_id text,
  p_game_id uuid
)
returns boolean
...
as $$
  select public.is_event_platform_sport(p_sport_id) and exists (
    ...
    where game.id = p_game_id
      ...
      and (
        (
          game.cloud_scope = 'personal'
          and game.created_by = (select auth.uid())
        )
        or (
          game.team_id is not null
          and public.current_team_role(game.team_id) in ('owner', 'admin')
        )
      )
  );
$$;
...
set search_path = public
as $$
  select public.can_manage_event_game('soccer', p_game_id);
$$;
...
as $$
  select public.is_event_platform_sport(p_sport_id) and exists (
    ...
    join public.game_event_canonical_publications publication
      on publication.game_id = game.id
    ...
    where game.id = p_game_id
      ...
      and public.can_track_team_games(game.team_id)
  );
$$;
...
set search_path = public
as $$
  select public.can_upload_final_event_audit(
    'soccer',
    p_game_id,
    p_recorded_by
  );
...
-- Trusted Soccer policy. Future sports add their own fixed policy function before receiving a
-- public finalization wrapper.
create or replace function public.is_soccer_primary_stream_ended(
  p_game_id uuid,
  p_recorded_by uuid
)
returns boolean
...
set search_path = public
as $$
  select * from public.get_event_finalization_readiness('soccer', p_game_id);
$$;

create or replace function public.get_event_canonical_publication(
  p_sport_id text,
  p_game_id uuid
)
returns table (
...
begin
  ...
  select
    ...
    coalesce(
      nullif(trim(primary_profile.display_name), ''),
      'StatKeeper user'
    )::text,
...
set search_path = public
as $$
  select * from public.get_event_canonical_publication('soccer', p_game_id);
$$;
...
begin
  ...
  return query select v_tracked_score, v_opponent_score;
end;
$$;
...
set search_path = public
as $$
  select public.finalize_event_game(
    'soccer',
    p_game_id,
    p_primary_recorded_by,
    p_event_revisions,
...
create or replace function public.reopen_event_game(
  p_sport_id text,
  p_game_id uuid,
  p_reason text
)
...
begin
  ...
  if not found then raise exception '% game not found', initcap(p_sport_id); end if;
  if v_game.status <> 'final' then
    raise exception '% game is not finalized', initcap(p_sport_id);
  end if;
...
set search_path = public
as $$
  select public.reopen_event_game('soccer', p_game_id, p_reason);
$$;

-- Soccer is entirely event-backed. Basketball requires canonical finalization only after an event
-- setup snapshot marks that specific cloud game; legacy aggregate Basketball remains unchanged.
...
set search_path = public
as $$
  select public.is_event_platform_sport(p_sport_id) and (
    p_sport_id = 'soccer'
    or exists (
      select 1
      from public.game_event_setup_snapshots setup
...
begin
  if tg_op = 'UPDATE' then
    if old.status = 'final' then
      if not (
        public.requires_canonical_event_finalization(old.sport_id, old.id)
        and new.status = 'in_progress'
        and not exists (
          select 1
          from public.game_event_canonical_publications publication
          where publication.game_id = old.id
            and publication.sport_id = old.sport_id
            and publication.invalidated_at is null
        )
      ) then
    ...
    if public.requires_canonical_event_finalization(old.sport_id, old.id)
      ...
      raise exception '% games must use canonical finalization', initcap(old.sport_id);
    end if;
  ...
  if new.home_team_player_id is not null
     and not exists (
       ...
       where player.id = new.home_team_player_id
         and player.is_team_placeholder
     ) then
  ...
  if new.opp_team_player_id is not null
     and not exists (
       select 1
       from public.players player
       where player.id = new.opp_team_player_id
         and player.is_team_placeholder
     ) then
...
create or replace function public.bind_event_game_v4(
  p_sport_id text,
...
begin
  ...
  if p_existing_game_id is null then
    v_binding := public.bind_event_game_v3(
      p_sport_id,
      null,
  ...
  where game.id = p_existing_game_id
    and game.sport_id = p_sport_id
  for share;
  if not found then raise exception '% game is unavailable', initcap(p_sport_id); end if;
  if v_game.status <> 'final' then
    v_binding := public.bind_event_game_v3(
      p_sport_id,
      p_existing_game_id,
  ...
  if not public.can_upload_final_event_audit(
    p_sport_id,
    v_game.id,
    v_user_id
  ) then
  ...
  if v_game.team_id is distinct from p_source_team_id
    ...
    raise exception 'Finalized % game binding is incompatible', lower(p_sport_id);
  end if;
...
begin
  ...
  from public.game_event_conflicts conflict
  join public.game_events event
    on event.id = conflict.event_id
    and event.game_id = conflict.game_id
    and event.recorded_by = conflict.recorded_by
    and event.sport_id = p_sport_id
...
begin
  ...
  if not public.can_manage_event_game(p_sport_id, v_conflict.game_id) then
    raise exception 'Team owner or admin access is required';
  end if;
  ...
  for share;
  if not found then
    raise exception '% game is unavailable for conflict resolution',
      initcap(p_sport_id);
  end if;
  if v_conflict.recorded_by is distinct from
      public.effective_event_primary_recorder(
        p_sport_id,
        v_conflict.game_id
      ) then
  ...
  if jsonb_typeof(v_selected) <> 'object'
     ...
     or (v_selected->>'sequence')::bigint is distinct from
       v_event.stream_sequence
     or (v_selected->>'createdAt')::timestamptz is distinct from
  ...
  into v_actors
  from jsonb_array_elements(v_selected->'actors')
    with ordinality actor(value, ordinality);
  ...
  perform public.record_access_audit_event(
    p_sport_id || '_primary_conflict_resolved',
    v_user_id,
...
begin
  ...
  for share;
  if not found then
    raise exception '% game is unavailable for checkpoint confirmation',
      initcap(p_sport_id);
  end if;
  if p_primary_recorded_by is distinct from
      public.effective_event_primary_recorder(p_sport_id, p_game_id) then
...
-- Reissue the shared event writer so finalized non-primary audit queues follow the stored
-- event-platform sport instead of a Soccer literal.
...
begin
  ...
  if not public.can_track_game(p_game_id) then
    ...
    from public.game_event_canonical_publications publication
    where publication.game_id = p_game_id
      and publication.sport_id = v_game_sport_id
      and publication.invalidated_at is null;
    ...
    if p_event_created_at > v_finalized_at
...
begin
  ...
  if not public.can_track_game(p_game_id)
     and not public.can_upload_final_event_audit(
       v_game_sport_id,
       p_game_id,
       v_user_id
     ) then
  ...
  where event.id = p_event_id
    ...
    and event.sport_id = v_game_sport_id;
...
-- Internal event-platform cores are intentionally unreachable through PostgREST. Existing
-- Soccer wrappers and already-generic recorder writers remain the only authenticated surface.
revoke all on function public.can_manage_event_game(text, uuid) from public;
revoke all on function public.can_manage_soccer_game(uuid) from public;
revoke all on function public.can_upload_final_event_audit(text, uuid, uuid) from public;
revoke all on function public.can_upload_final_soccer_audit(uuid, uuid) from public;
revoke all on function public.is_soccer_primary_stream_ended(uuid, uuid) from public;
revoke all on function public.get_event_finalization_readiness(text, uuid) from public;
revoke all on function public.get_event_canonical_publication(text, uuid) from public;
revoke all on function public.validate_soccer_finalization_policy(uuid, uuid) from public;
revoke all on function public.finalize_event_game(text, uuid, uuid, jsonb, text, jsonb) from public;
revoke all on function public.reopen_event_game(text, uuid, text) from public;
revoke all on function public.requires_canonical_event_finalization(text, uuid) from public;
revoke all on function public.enforce_game_identity_and_final_state() from public;
revoke all on function public.bind_event_game_v4(
  text, uuid, text, uuid, uuid, text, text, text, date, jsonb, jsonb
) from public;
revoke all on function public.get_event_primary_conflicts_for_finalization(text, uuid)
  from public;
revoke all on function public.resolve_event_primary_conflict_for_finalization(
  text, uuid, text
) from public;
revoke all on function public.confirm_event_primary_checkpoint_for_finalization(
  text, uuid, uuid, integer, jsonb, integer, bigint, text
) from public;
...
grant execute on function public.get_soccer_primary_conflicts_for_finalization(uuid)
  to authenticated;
revoke all on function public.resolve_soccer_primary_conflict_for_finalization(uuid, text)
  from public;
grant execute on function public.resolve_soccer_primary_conflict_for_finalization(uuid, text)
  to authenticated;
...
revoke all on function public.record_game_event_conflict(uuid, uuid, jsonb, jsonb)
  from public;
grant execute on function public.record_game_event_conflict(uuid, uuid, jsonb, jsonb)
  to authenticated;
...
comment on table public.game_event_canonical_publications is
  'Append-only canonical publications for event-platform sports; reopen invalidates active rows.';
comment on function public.finalize_soccer_event_game(
  uuid, uuid, jsonb, text, jsonb
) is
  'Permanent Soccer finalization wrapper over trusted event-platform publication mechanics.';
comment on function public.reopen_soccer_event_game(uuid, text) is
  'Permanent Soccer reopen wrapper preserving append-only canonical publication history.';
